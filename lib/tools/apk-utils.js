...
apkUtilsMethods.getInstallXpathLocator = function (manufacturer, language) {
  ...
  if (manufacturer && xpathLocators.hasOwnProperty(manufacturer)) {
    lan = language ? xpathLocators[manufacturer][language] : xpathLocators[manufacturer].zh;
    retry = xpathLocators[manufacturer].retry ? xpathLocators[manufacturer].retry : xpathLocators.def.retry;
    return [lan, retry];
  }
...
apkUtilsMethods.forceInstall = async function (androidDriver) {
  ...
  let [locator, retries] = this.getInstallXpathLocator(manufacturer, language);
