...
const logTestwa = logger.getLogger('Testwa');
let apkUtilsMethods = {};
...
apkUtilsMethods.startApp = async function (startAppOptions = {}) {
  try {
    ...
    if (stdout.indexOf("Error: Activity class") !== -1 &&
      ...
      // Assume Success
        this.appLaunchTotalTime = stdout.match(/TotalTime: (.*)/)[1] ? + stdout.match(/TotalTime: (.*)/)[1] : "无法获取";
    }
...
apkUtilsMethods.installFromDevicePath = async function (androidDriver, apkPathOnDevice, opts = {}) {
  let func = async function (adb) {
    let stdout = await adb.shell(['pm', 'install', '-r', apkPathOnDevice], opts);
  ...
  if (!androidDriver) {
    ...
    //testwa: force install
    await Promise.all([func(this), this.forceInstall(androidDriver)])
  }
};

apkUtilsMethods.install = async function (androidDriver, apk, replace = true, timeout = 60000) {
  if (replace) {
    ...
    try {
      if (!androidDriver) {
       ...
       //testwa: force install
        await Promise.all([this.adbExec(['install', apk], {timeout}), this.forceInstall(androidDriver)]);
      }
    } catch (err) {
...
apkUtilsMethods.getInstallXpathLocator = function (manufacturer, language) {
  if (manufacturer && xpathLocators.hasOwnProperty(manufacturer)) {
    return language ? xpathLocators[manufacturer][language] : xpathLocators[manufacturer].zh;
  }
  return language ? xpathLocators.def[language] : xpathLocators.def.zh;
};

apkUtilsMethods.forceInstall = async function (androidDriver) {
  logTestwa.debug("Forcing install...");
  let manufacturer = androidDriver.caps.deviceManufacturer.toLowerCase();
  let language = androidDriver.caps.language.toLowerCase();
  let locator = this.getInstallXpathLocator(manufacturer, language);
  logTestwa.debug(`Using xpath to locate install button : ${locator}`);
  ...
  // wait for 2000ms for install modal to popup
  setTimeout(func, 2000);
};

apkUtilsMethods.installOrUpgrade = async function (androidDriver, apk, pkg = null, timeout = 60000) {
  let apkInfo = null;
  if (!pkg) {
  ...
  if (!await this.isAppInstalled(pkg)) {
    await this.install(androidDriver, apk, false, timeout);
    return;
  }
  ...
  log.debug(`The installed "${pkg}" package is older than ${apk} (${pkgVersionCode} < ${apkVersionCode}). ` +
            `Executing upgrade`);
  try {
    await this.install(androidDriver, apk, true, timeout);
  } catch (err) {
