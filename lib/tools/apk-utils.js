...
apkUtilsMethods.installFromDevicePath = async function (androidDriver, apkPathOnDevice, opts = {}) {
  let func = async function (adb) {
    log.debug('before install func... testwa');
    let stdout = await adb.shell(['pm', 'install', '-r', apkPathOnDevice], opts);
    if (stdout.indexOf("Failure") !== -1) {
      log.errorAndThrow(`Remote install failed: ${stdout}`);
    }
  };
  log.debug('before install... testwa');
  await Promise.all([func(this), this.forceInstall(androidDriver, '//android.widget.Button[@text="Install"]')])
};
  ...
  if (replace) {
    ...
    try {
      if (!androidDriver) {
       ...
       // force install
        await Promise.all(this.adbExec(['install', apk], {timeout}), this.forceInstall(androidDriver, '//android.widget.Button[@text="Install"]'));
      }
    } catch (err) {
...
apkUtilsMethods.forceInstall = async function (androidDriver, locator) {
  log.debug("Forcing install...");
